...
static const char GETMODELSCANS[] = "household_objects_database_msgs/GetModelScans";
  ...
  class GetModelScansResponse : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (this->matching_scans.size() >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->matching_scans.size() >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (this->matching_scans.size() >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (this->matching_scans.size() >> (8 * 3)) & 0xFF;
      offset += 4;
      for( uint32_t i = 0; i < matching_scans.size(); i++){
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += 4;
      matching_scans.resize(matching_scans_lengthT);
      for( uint32_t i = 0; i < matching_scans.size(); i++){
      offset += this->matching_scans[i].deserialize(inbuffer + offset);
      }
     return offset;
    }
