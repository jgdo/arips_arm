#ifndef _ROS_object_recognition_msgs_RecognizedObjectArray_h
#define _ROS_object_recognition_msgs_RecognizedObjectArray_h

#include <stdint.h>
#include <string.h>
#include <stdlib.h>
#include <array>
#include <vector>
#include "ros/msg.h"
#include "std_msgs/Header.h"
#include "object_recognition_msgs/RecognizedObject.h"

namespace object_recognition_msgs
{

  class RecognizedObjectArray : public ros::Msg
  {
    public:
      typedef std_msgs::Header _header_type;
      _header_type header;
      std::vector<object_recognition_msgs::RecognizedObject> objects;
      std::vector<float> cooccurrence;

    RecognizedObjectArray():
      header(),
      objects(),
      cooccurrence()
    {
    }

    virtual int serialize(unsigned char *outbuffer) const
    {
      int offset = 0;
      offset += this->header.serialize(outbuffer + offset);
      *(outbuffer + offset + 0) = (this->objects.size() >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->objects.size() >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (this->objects.size() >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (this->objects.size() >> (8 * 3)) & 0xFF;
      offset += 4;
      for( uint32_t i = 0; i < objects.size(); i++){
      offset += this->objects[i].serialize(outbuffer + offset);
      }
      *(outbuffer + offset + 0) = (this->cooccurrence.size() >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->cooccurrence.size() >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (this->cooccurrence.size() >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (this->cooccurrence.size() >> (8 * 3)) & 0xFF;
      offset += 4;
      for( uint32_t i = 0; i < cooccurrence.size(); i++){
      union {
        float real;
        uint32_t base;
      } u_cooccurrencei;
      u_cooccurrencei.real = this->cooccurrence[i];
      *(outbuffer + offset + 0) = (u_cooccurrencei.base >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (u_cooccurrencei.base >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (u_cooccurrencei.base >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (u_cooccurrencei.base >> (8 * 3)) & 0xFF;
      offset += sizeof(this->cooccurrence[i]);
      }
      return offset;
    }

    virtual int deserialize(unsigned char *inbuffer)
    {
      int offset = 0;
      offset += this->header.deserialize(inbuffer + offset);
      uint32_t objects_lengthT = ((uint32_t) (*(inbuffer + offset))); 
      objects_lengthT |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1); 
      objects_lengthT |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2); 
      objects_lengthT |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3); 
      offset += 4;
      objects.resize(objects_lengthT);
      for( uint32_t i = 0; i < objects.size(); i++){
      offset += this->objects[i].deserialize(inbuffer + offset);
      }
      uint32_t cooccurrence_lengthT = ((uint32_t) (*(inbuffer + offset))); 
      cooccurrence_lengthT |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1); 
      cooccurrence_lengthT |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2); 
      cooccurrence_lengthT |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3); 
      offset += 4;
      cooccurrence.resize(cooccurrence_lengthT);
      for( uint32_t i = 0; i < cooccurrence.size(); i++){
      union {
        float real;
        uint32_t base;
      } u_cooccurrencei;
      u_cooccurrencei.base = 0;
      u_cooccurrencei.base |= ((uint32_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_cooccurrencei.base |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_cooccurrencei.base |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_cooccurrencei.base |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3);
      this->cooccurrence[i] = u_cooccurrencei.real;
      offset += sizeof(this->cooccurrence[i]);
      }
     return offset;
    }

    const char * getType(){ return "object_recognition_msgs/RecognizedObjectArray"; };
    const char * getMD5(){ return "bad6b1546b9ebcabb49fb3b858d78964"; };

  };

}
#endif