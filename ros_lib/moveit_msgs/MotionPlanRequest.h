...
namespace moveit_msgs
{
  ...
  class MotionPlanRequest : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (this->goal_constraints.size() >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->goal_constraints.size() >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (this->goal_constraints.size() >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (this->goal_constraints.size() >> (8 * 3)) & 0xFF;
      offset += 4;
      for( uint32_t i = 0; i < goal_constraints.size(); i++){
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += 4;
      goal_constraints.resize(goal_constraints_lengthT);
      for( uint32_t i = 0; i < goal_constraints.size(); i++){
      offset += this->goal_constraints[i].deserialize(inbuffer + offset);
      }
