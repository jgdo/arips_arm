...
namespace moveit_msgs
{
  ...
  class RobotState : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (this->attached_collision_objects.size() >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->attached_collision_objects.size() >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (this->attached_collision_objects.size() >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (this->attached_collision_objects.size() >> (8 * 3)) & 0xFF;
      offset += 4;
      for( uint32_t i = 0; i < attached_collision_objects.size(); i++){
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += 4;
      attached_collision_objects.resize(attached_collision_objects_lengthT);
      for( uint32_t i = 0; i < attached_collision_objects.size(); i++){
      offset += this->attached_collision_objects[i].deserialize(inbuffer + offset);
      }
      union {
        bool real;
        uint8_t base;
