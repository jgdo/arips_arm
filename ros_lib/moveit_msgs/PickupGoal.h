...
namespace moveit_msgs
{
  ...
  class PickupGoal : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (this->possible_grasps.size() >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->possible_grasps.size() >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (this->possible_grasps.size() >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (this->possible_grasps.size() >> (8 * 3)) & 0xFF;
      offset += 4;
      for( uint32_t i = 0; i < possible_grasps.size(); i++){
      ...
      *(outbuffer + offset + 0) = (this->attached_object_touch_links.size() >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->attached_object_touch_links.size() >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (this->attached_object_touch_links.size() >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (this->attached_object_touch_links.size() >> (8 * 3)) & 0xFF;
      offset += 4;
      for( uint32_t i = 0; i < attached_object_touch_links.size(); i++){
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += 4;
      possible_grasps.resize(possible_grasps_lengthT);
      for( uint32_t i = 0; i < possible_grasps.size(); i++){
      offset += this->possible_grasps[i].deserialize(inbuffer + offset);
      }
      ...
      offset += 4;
      attached_object_touch_links.resize(attached_object_touch_links_lengthT);
      for( uint32_t i = 0; i < attached_object_touch_links.size(); i++){
      uint32_t length_attached_object_touch_linksi;
      arrToVar(length_attached_object_touch_linksi, (inbuffer + offset));
      offset += 4;
      for(unsigned int k= offset; k< offset+length_attached_object_touch_linksi; ++k){
          inbuffer[k-1]=inbuffer[k];
      }
      inbuffer[offset+length_attached_object_touch_linksi-1]=0;
      this->attached_object_touch_links[i] = (char *)(inbuffer + offset-1);
      offset += length_attached_object_touch_linksi;
      }
      union {
        bool real;
