...
static const char GETCARTESIANPATH[] = "moveit_msgs/GetCartesianPath";
  ...
  class GetCartesianPathRequest : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (this->waypoints.size() >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->waypoints.size() >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (this->waypoints.size() >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (this->waypoints.size() >> (8 * 3)) & 0xFF;
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += 4;
      waypoints.resize(waypoints_lengthT);
      for( uint32_t i = 0; i < waypoints.size(); i++){
