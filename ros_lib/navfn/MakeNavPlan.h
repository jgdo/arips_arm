...
static const char MAKENAVPLAN[] = "navfn/MakeNavPlan";
  ...
  class MakeNavPlanResponse : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (this->path.size() >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->path.size() >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (this->path.size() >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (this->path.size() >> (8 * 3)) & 0xFF;
      offset += 4;
